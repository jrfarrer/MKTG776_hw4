...
bb_expected %>%
  ...
  mutate(p.value = pchisq(chisq, df = 6 - if_else(str_detect(model, "Inflated"), 1L, 0L) - 1, lower.tail = FALSE)) %>%
...
we see that $t$ gets bigger, $x$ is the primary driver of the posterior estimate rather than the population mean $\frac{r}{\alpha} = `r (billboard_r / billboard_alpha)`$. The actual distribution gamma distribution of the posterior estimates are shown below:
